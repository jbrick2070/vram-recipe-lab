...
2. **GPU idle & Host RAM tracking** — `nvidia-smi` shows under **2.5 GB (2560 MB)** allocated VRAM. System Host RAM (`psutil.virtual_memory()`) is recorded as baseline host RAM and tracked concurrently at 200ms intervals throughout execution. If another process is using the GPU (> 2.5 GB allocated), abort; do not queue behind it.
...
8. **Fixtures uploaded** — required fixtures are present on disk and uploaded to server input directory via `POST /upload/image`. Never regenerate a fixture.
