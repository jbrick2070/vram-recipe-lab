...
Run this through `mux_source_delivery.py`, which maps diagnostic video plus the
original source audio, copies video, encodes AAC, limits delivery to 3.88 s, and
writes `results/delivery/<recipe>.json`. Preserve a mux receipt and prove the
video stream hash did not change. Media
...
render. `ESCALATE.md` is an explicitly human-readable close-out, not a machine
certification receipt; it must cite the selected artifact/receipt, recipe and
artifact hashes, 15.04 GiB peak, attempt history and reason for stopping.
...
post-minus-pre settled delta while the same verified 8199 server remains live.
Fail on any invalid child, >14.5 GiB peak, marginal result, non-warm second
identity, or >0.25 GiB rise in candidate repeat or sentinel absolute peak or
post-settle median. Net peak remains required, finite and receipt-bound as
per-run diagnostic/clamp evidence, but is not compared across cache-classic
children because their pre-run resident baselines can legitimately differ.
Record general failures separately from actual VRAM-creep failures. Every
S1/S2/S3 absolute-peak and post-settle comparison uses S0 as its fixed reference
envelope; candidate repeats compare with their own first run.
...
The live lane check is bidirectional: when the manifest requests no-pinned, the
argv must contain `--disable-pinned-memory`; when false, argv must omit it. This
suite uses only the true/no-pinned case and never restarts to test another mode.
