...
2. Add a fail-closed runner CLI option for the completion window, defaulting to the existing 1,800 seconds. Accept one integer exactly once in a bounded range up to 7,200 seconds and record the selected value as `completion_timeout_s` in each new schema-v3 receipt; schema v3 is extensible, so no version bump is needed. Historical schema-v3 receipts may omit the field. Attempt-005 verification requires the exact selected value in its new receipts. Attempt-005 passes 3,600 seconds for pair 10 and 5,400 seconds for pair 11; all ordinary callers retain 1,800 seconds. Add focused parser/default/receipt tests.
3. Add an attempt-005 recovery mode to the existing H3 campaign coordinator (preferred over a parallel ad-hoc renderer). Give it exact id `h3music-20260810T023023Z-97ca44b2-attempt-005`, launcher `scratch/start_h3_music_attempt5.ps1`, launch recorder `scratch/record_h3_music_attempt5_operator_launch.py`, exact attempt-004 lifecycle/recovery pins, and require a clean owned-lab state before appending any attempt-005 lifecycle row. Generalize the operator argv/transport/process validators for this exact attempt without weakening attempt-004 checks.
4. Carry pairs 1-9 with zero executions. Extend `pair_run_schedule()` so pair 10 is run 2 cold/config 1 and run 3 warm/config 2, allowed receipts 1-3, preserving failed run 1; pair 11 remains run 1 cold/config 1 and run 2 warm/config 2. Extend `ensure_pristine_pair()` to accept only the exact pinned pair-10 run-1 receipt/alias and zero artifacts. Set the runbook nonce/count branches to exactly four unique attempt-005 executor nonces.
...
6. Preserve attempt-004 operator stdout/stderr without manufacturing a success receipt for it. The recovery receipt pins only historical attempt-004 inputs and does not depend on not-yet-created attempt-005 transport files. After that receipt exists, create the named attempt-005 launcher/recorder and atomically claim the new operator directory; on terminal exit, create a launch receipt bound to process identity, argv, stdout/stderr identities, the lifecycle hash chain, and the terminal event.
