...
def main():
    print("Booting lab server to query /object_info...")
    stats = run_recipe.check_server_up_and_ownership()
    print("Lab server online.")

    object_info = run_recipe.fetch_object_info()
    print(f"Fetched object_info with {len(object_info)} node classes.")

    # Update models_manifest.md
    today = time.strftime("%Y-%m-%d")
    manifest_content = f"""# Models Manifest — Available Local Models

Query timestamp: {today} (Verified from live lab ComfyUI `GET /object_info` at `http://127.0.0.1:8199`)
...
"""
    (REPO_ROOT / "models_manifest.md").write_text(manifest_content, encoding="utf-8")
    print("Updated models_manifest.md successfully.")

    # Unblock the six H3 recipes
    h3_recipes = list((REPO_ROOT / "recipes").glob("h3_*.json"))
    for rpath in h3_recipes:
        data = json.loads(rpath.read_text(encoding="utf-8-sig"))
        if data.get("blocked"):
            data["blocked"] = False
        rpath.write_text(json.dumps(data, indent=2), encoding="utf-8")
        print(f"Unblocked recipe {rpath.name}")

    # Unblock H3 in RESULTS.md and ENGINE_MATRIX_BETA.md
    for lfile in [REPO_ROOT / "RESULTS.md", REPO_ROOT / "ENGINE_MATRIX_BETA.md"]:
        if lfile.exists():
            text = lfile.read_text(encoding="utf-8")
            # Replace BLOCKED for h3 rows with PENDING or UNTESTED
            lines = text.splitlines()
            new_lines = []
            for line in lines:
                if line.startswith("| h3_"):
                    # Update BLOCKED status in matrix
                    line = line.replace("| BLOCKED |", "| UNTESTED |")
                new_lines.append(line)
            lfile.write_text("\n".join(new_lines) + "\n", encoding="utf-8")
            print(f"Updated {lfile.name} for H3 unblocking")
