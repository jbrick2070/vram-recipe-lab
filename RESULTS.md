...
| h3_r2v_refaudio_static_control | UNRENDERED/HELD | 0.00 | 0.00 | 0.0 | No run; the TTS smoke is the only RefAudio cell rendered so far |
| h3_r2v_refaudio_music_opening | UNRENDERED/HELD | 0.00 | 0.00 | 0.0 | No run; the TTS smoke is the only RefAudio cell rendered so far |
| h3_mime_i2v | GATE PASS (cold-only); HUMAN APPROVED CONTINUATION; FORMAL EAR FIELDS PENDING | 7.28 | 2.52 | 178.9 | Run #1 only; exact 90 frames/3.750 s with native audio at -27.5 LUFS. Jeffrey approved one R2V continuation; the required one-line soundscape description remains pending. |
| h3_mime_r2v | GATE PASS (cold-only); HUMAN EYE/EAR PENDING | 7.23 | 2.61 | 188.3 | Run #1 only; exact 90 frames/3.750 s, strong objective portrait stability, native audio about -40.5 LUFS; final authorized mime render, not warm or promoted |
