...
def validate_command(command: Sequence[str], engine: str) -> dict[str, Any]:
    ...
    if _flag_value(command, "--engine") is not None:
        raise SidecarError(
            "--engine is inert in episode mode; engine must be the server-bound OTR_FORCE_ENGINE_MAP"
        )
    return {
        "argv": list(command),
        "python": str(python),
        ...
        "engine": engine,
        "server_bound_force_map": f"*={engine}",
        "bundle": str(bundle),
...
def run(
    ...
    *,
    ...
    if args.expected_outcome == "either" and not (success_found or refusal_found):
        errors.append("Neither successful replay nor S4 refusal was proved")
